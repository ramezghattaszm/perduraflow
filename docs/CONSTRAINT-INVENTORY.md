...
### Silent gaps surfaced by the inventory pass (were untracked — now logged)
> Surfaced by the read-only inventory cross-check (spec vs. built vs. tracker). Each was spec'd but neither built nor logged; verified by grepping the codebase for the relevant entity. Ranked by consequence. Cross-referenced from `REMAINING-ITEMS.md` § "Silent gaps surfaced by inventory pass".
- ❌ **D11 — Alternate routing selection (HIGH, was SILENT).** Spec: Scheduling §3/§5.2/§6.2 + committed-schedule contract §4.4 (`routing_id` = *"primary or alternate, D11"*). **Not built:** no `preference_rank` (routing carries only `is_primary`); the scheduler always resolves `resolveRouting(..., { primaryOnly: true })` ([scheduling.service.ts:856](apps/api/src/modules/scheduling/scheduling.service.ts#L856), [simulator.service.ts:139](apps/api/src/modules/scheduling/simulator.service.ts#L139)); the line-down "reroute" lever swaps a **resource within the eligible group**, not a **routing**. **Contract misrepresentation:** the output contract records `routing_id` as possibly-alternate while the engine only ever emits primary. **Consequence:** the optimizer cannot route around a bottleneck / downed machine via an alternate process.
- ❌ **JIS — just-in-sequence sequencing (HIGH, was SILENT).** Spec: Scheduling §2.3, §4.1 (`jis_sequence_number`, `jis_line_side_time`, `jis_broadcast_id`), §4.4, §6.2. **Not built:** none consumed in `scheduling`; `demand_type` is stored/mapped but never drives sequencing. Only the NR4 net-req pass-through mentions JIS (itself SKIP-10). **Consequence:** a headline automotive delivery mode is unmodeled in the scheduler; slipped through because the demo is JIT/stock.
- ⚠️ **D6 / §4.6 decision-audit trace (MEDIUM, partial — was under-tracked).** Per committed version the spec requires `constraint_set_ref`, `ml_model_versions[]`, `llm_interaction_refs`. **Present:** `objective_summary`, `master_data_asof`. **Missing:** the other three (zero hits); live LLM interactions are not linked to committed versions. SKIP-22 (generic audit framework) does **not** name this projection. **Consequence:** the IATF "reconstruct why a committed plan was chosen" claim is only partially satisfiable (no model-version pin, no constraint-set ref, no LLM-influence link).
- ⚠️ **§4.11 — proposal-disposition record (MEDIUM, was SILENT).** Requires `selected_option_rank`, `selected_vs_top`, `edited_before_commit`, `reason_code`, `approver_id`, `committed_schedule_version_id`, `outcome_ref` → feeds §14.2 AI-performance KPIs. **Present:** `what_if_result` stores the ranked options + recommended + `createdBy`; **none** of the human-disposition fields. **Consequence:** "AI suggestion vs. actual" is a lost signal; the §14.2 / §1003 AI-performance KPIs have no source data.
- ⚠️ **D23/D38 — customer/program firm fence not consumed (MEDIUM/low, was unlogged).** `firm_fence_days` is seeded + admin-editable but has **zero reads** in `scheduling`/`app` — the engine never reads it (per-line `firmness` is the operative signal, so the fence is **partly redundant**). Logged as unimplemented-and-partly-redundant.
- ❌ **§5.2 — operation `move_time` / `queue_time` not consumed (LOW, was unlogged).** Optional operation fields; the sequencer places ops back-to-back with no inter-op transfer/queue slack. Logged so it isn't mistaken for modeled behavior.
