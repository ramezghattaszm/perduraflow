...
- [ ] **At-risk remediation as a direct costed option-set (not only "Evaluate options → Copilot").** A firm at-risk order's "Evaluate options" opens the Copilot conversationally; ideally it renders SELECTABLE costed options directly (like the line-down what-if card: reroute / OT / expedite / re-promise / assign-faster-operator, costed, recommendation marked, honest-unachievable preserved). The engine already PRODUCES the root-matched options (the `at_risk_remediation` + standing-reroute + **operator-rooted faster-operator/OT/reroute, built this session**); the gap is the SURFACE — render them as a pick-from-list reachable in one click from the at-risk order. The faster-operator option now rides this same path (labelKey `whatif.option.fasterOperator`).
- [ ] **Demo-data: the operator-root scenario isn't naturally triggered by the warm-start seed.** Part A (operator as a lateness root) + Part B (faster-operator lever) are built + live-verified, but the canonical `demo:reset` state never roots an order at `operator` — Ana at 0.85 is too fast to overflow, and any op slow enough to go late on busy Press A roots at `capacity`/`resource` (queue) instead. A clean operator root needs an op that starts FRESH (origin/release/working_window-bound) AND overflows its due from its own slow run. To make Part A+B visible in the demo without manual DB pokes, tune the seed (e.g. a dedicated firm Press A order with a tight due + Ana slow enough, OR a scripted scenario) so one order reliably roots `operator`. **Decision for RG** (demo narrative): how slow is Ana / which order carries the story.
