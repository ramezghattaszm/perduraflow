...
- **Cost as an optimization objective — NOW IN (C6 shipped, wi-9..wi-12).** *(Superseded the original "computed and reported but not scored" boundary.)* `costPerUnit` is a **weighted scoring factor** (`whatif.scoring.ts`, weight 4 in `aps-w2`, below lateness so firm-lateness dominance holds: 10 ≥ 2×4). It sums **resource cost** (setup/run/overhead, Tier-B rates) + **OT premium** + **operator LABOR** (`laborRate · working-hours`, wi-12) over costed units, with a non-null guard (uncosted plan → 0, never NaN). So competing remediation levers (faster-operator vs OT vs reroute) rank on real $ — the faster-but-pricier operator's labor is no longer invisible. What remains a *roadmap* refinement (not a boundary): richer cost models (energy, scrap, tooling-life, tiered labor/shift premiums) — see Part 3.
...
For an evaluated plan: **score = Σ (rawValue × weight)** over six factors; **lower is better** (matches the sequencer). All factors are penalties (positive = worsens).
...
| cost | `costPerUnit` (resource + OT + operator labor) | 4 | penalty |

**KPIs** (reported alongside, not all scored): `otif`, `costPerUnit` (**scored** — see the cost factor; surfaced as a KPI too), `oee` (**null in plan-scoring** — execution-only), `lateOrders`, `throughput`, `churn` (displacement ratio).
...
- **Cost as an optimization objective — SHIPPED (C6, wi-9..wi-12).** Cost is now a **weighted factor** (weight 4, below lateness — firm-lateness dominance survives: 10 ≥ 2×4). Done in three moves: calibration ($/unit folds into the score at weight 4); non-null guard (uncosted → 0); operator labor folded in (wi-12: `laborRate · working-hours`) so cross-lever remediation ranks on real $. The talk track is now "cost-optimized," not "cost-aware." What remains *roadmap*: richer cost models — energy/scrap/tooling-life, tiered labor & shift premiums, OT charged at a true premium multiple rather than the run-rate proxy.
...
The engine is a **deterministic, auditable constraint heuristic** with an honest, explicit boundary: it places work against working-time, material, precedence, inspection-capacity, and minimum-batch constraints; it scores plans against a six-factor objective (firm-lateness dominant) that now includes cost ($/unit — resource + OT + operator labor, weight 4, below lateness); and it deliberately keeps crew-allocation optimization, multi-level BOM, and global re-optimization out — each documented with its production path. The ML and LLM layers *feed* and *explain* the engine; they never decide. The production build replaces the heuristic with a real optimizer behind an existing seam, makes the weights hierarchical config, completes the deferred constraints, and builds the real ML engine — all without changing the authority model or the determinism contract that make the system auditable.
