...
| **Status** | Draft v0.4 |
| **Date** | 2026-06-14 |
| **Companion docs** | Platform architecture specification (Draft v0.10); Production scheduling module business & functional specification (Draft v0.11) |
...
> **Status note:** MD1–MD15 are **Agreed**. The MDQ question log (Section 9) is fully resolved (MDQ1–MDQ6).
...
| **MD15** | **Skill/certification taxonomy, operator qualifications, and order economics join the Master-Data canonical view (externally sourced).** Two cross-module reference sets that the platform consumes but does not author: (1) the **skill & certification taxonomy** and **operator→qualification** records (which operators hold which certifications), plus an optional **labor-rate** attribute — the canonical reference behind the scheduler's certification-grain constraint (scheduling D54) and the labor-cost KPI (D57); and (2) **order/customer economics** — contribution margin, penalty exposure, customer priority — the canonical reference behind allocation-by-priority (NMA, intra-plant order allocation). Both are **externally sourced** (HR system; commercial/ERP) via the integration layer (D35) and held as a **canonical view only** — the external-system mappings live in the integration/mapping layer, not here (the existing canonical-vs-mapping separation). Master Data does **not** roster operators or set commercial policy; it holds the reference data those decisions read. | Per the ownership principle (MD12), certifications and order economics have clear cross-module use (scheduling, allocation, future labor/maintenance and commercial modules all read them), so they belong in the canonical view rather than being fragmented into each consumer. Keeping them externally sourced and mapping-separated preserves the canonical-only role and avoids the platform becoming the system of record for HR or commercial data. | Agreed |
