...
| **SKIP-04** | ML parameter prediction | **Phase 3 — closed loop LIVE:** the `learning` module ingests execution actuals and derives **damped** learned cycle/setup (windowed snap-on-gate + guardrails, A18); on re-solve the sequencer overlays held values → `*_source` flips `standard→ml_adjusted` + `*_confidence` populates, **zero schema/board change** (fields exist from `0004`). The learner is a deterministic rule, not a *trained model* yet. | **Trained** models (changeover/cycle/downtime/scrap) + Phase-4 prediction-ahead on this mechanism; confidence-driven `ml_reliance` trigger | D3 / D5 / D7 / D41 / A14 · Phase 4 | In progress (closed loop + damped learner built; trained models / prediction deferred to Phase 4) |
...
| **SKIP-14** | Labor scheduling & optimization | Machine-paced default (labor not binding); labor pools seeded only where an op is labor-constrained. **Certification-grain constraint + confirmed fill proposal (D54) is in scope** and demoed (Collision 4), synthetic. **Operator performance (C5) is in scope** as a *consumed* input: a pinned `resource_operator_assignment` (scheduler-owned §4.8) + an `operator.performanceFactor` ("percent of standard") divide the op's **run time** (`effectiveCycle = baseCycle / performanceFactor`, setup untouched) at the existing cycle seam — `ENGINE_VERSION` wi-7, threads to what-if/baselines. The scheduler **reads** the assignment, never assigns/optimizes it (rostering stays external). Factor lives on the operator now; task-specific factor-on-assignment + recurring shift windows are documented future refinements | Rostering/optimization: if ever built, a fully separate module; individual operator rostering + operator→station assignment stays external. The labor-availability/**certification constraint** (D54, consuming MD15) is **not** deferred — it's built as a consumed input; **operator performance (C5)** likewise rides as a consumed pinned assignment | D29 / D43 / D54 · MD15 | Deferred (rostering only; cert-constraint + operator-performance in scope) |
...
| **SKIP-51** | Integration / actuals source | **Phase 3 BUILT:** seeded/deterministic actuals **simulator** (demo fixture in `scheduling`, dev-only `POST /dev/scheduling/simulate` + drift trigger) emitting 4.3 events on the EventBus + manual `POST /learning/actuals`; cleanly swappable for a real connector behind the same event. | Real connectors per the three D35 binding modes; MES real-time actuals feed | D35 / §13 · connector build | In progress (simulator built; real connectors deferred) |
| **SKIP-52** | Calendar admin editors (presentation) | Shift patterns / holidays / working days are edited via raw **JSON editors** in the admin Calendars screen (phase-0 minimal UI); the engine reads the calendar fully — only the editing UX is raw. Code refs: [calendar.schema.ts:9](apps/api/src/modules/org/schema/calendar.schema.ts#L9), [calendars-screen.tsx:34](packages/app/features/admin/calendars/calendars-screen.tsx#L34) | Structured form controls — shift-pattern builder, weekday multi-toggle, holiday picker — replacing the JSON editors | D17 · admin UI polish (relates to REMAINING-ITEMS §G#1 `workingDays` form) | Deferred (JSON editors; structured form later) |
| **SKIP-53** | Tenant logo / branding (presentation) | Tenant `logo_url` is nullable → `OrgAvatar` renders a deterministic initials placeholder; the repo ships only the placeholder, real logos are tenant-supplied later. Code refs: [tenant.schema.ts:14](apps/api/src/modules/tenant/schema/tenant.schema.ts#L14), [seed.ts:93](apps/api/src/db/seed.ts#L93), [OrgAvatar.tsx:17](packages/ui/src/OrgAvatar.tsx#L17) | Tenant-managed logo upload/URL (branding admin); real logos at runtime via `logo_url` | Tenant admin / branding | Deferred (placeholder only) |
| **SKIP-54** | Narration surface (A19) | **Phase 5 BUILT:** a `@Global` **`llm`** kernel module (LLMGateway + **backend-agnostic provider adapter**) renders the structured rationale (D55) to prose — **translate-only**, async, non-blocking, never in the commit path; `recorded` provider (deterministic, the provable translate-only baseline) is the default. | Bedrock/managed-provider adapters (the `anthropic` adapter body is pending interface sign-off); narration is already available to any module emitting structured rationale | A19 / D55 · LLMGateway providers | **Built** (recorded default; real-provider adapters additive) |
| **SKIP-55** | Measured-historical baseline | **Phase 5 BUILT:** **both** D57 arms — `frozen_engine_snapshot` (computed on demand) **and** `measured_historical` (aggregates seeded `historical_outcome` rows; honest empty-state for no-history scopes). | Swap the seeded rows for a real MES/historian feed (`source: 'mes'`, same row shape, zero code change) | D57 / D35 · pilot data | **Built** (seeded representative; real-data swap is a row change) |
| **SKIP-63** | What-if + apply (D55/D26, Phase 5 BUILT) | What-if engine inside `scheduling`: change-set-general, **deterministic + feasibility-honest** option-set with structured rationale (factor/option/constraint-addressable; `weightSetVersion`-pinned, persisted as the Phase-6 substrate); **Apply** = human action → new draft. 8 API proofs + browser-verified. | Phase 6 drives the same engine with **conversational/open** change-sets + grounds answers in the stored rationale (no engine rework) | D55 / D26 · Phase 6 | **Built** (defined triggers; conversational layer deferred to Phase 6) |
| **SKIP-60** | Prediction model (Phase 4 BUILT) | **OLS linear-trend** extrapolation on the actuals series → threshold-crossing + horizon-degraded confidence (deterministic, damped, bounded) | A **real predictive model** (ARIMA / ML regression, SageMaker A14) behind the same `learning.read` prediction surface — the linear fit is the honest placeholder (as the greedy heuristic stands in for the optimizer) | A14 ML serving · pilot history | Deferred (placeholder model) |
| **SKIP-61** | Prediction-accuracy measure (Phase 4→5) | `parameter_prediction` retains the forecast + `outcome` (materialized/corrected); the measure is **not** computed | A **forecast-accuracy** metric (did the prediction come true; calibration over time) feeding A16/A17 trust graduation | D5 closed loop · Phase 5 | Deferred (data retained, not computed) |
| **SKIP-62** | Autonomy gate breadth (Phase 4) | Confidence×tier gate **BUILT + proven**: Tier-1 auto / Tier-3 always-human; **Tier-2 bounded-auto** is a config seam (advisory-first default); objective trade-off weights are a Phase-5 View-5 seam | Full Tier-2 bounded-auto policy tuning + objective-weight config (service floor / OT / churn / expedite) + priority-tier UI | A18 Tier-2 · D42 · Phase 5 | Deferred (seam) |
| **SKIP-64** | Conversational layer (Phase 6 BUILT) | **Agentic tool-loop in the LLM gateway** + a `ConversationService` (in `scheduling`): routing-by-tool-selection (Type-1 `retrieve_what_if` / Type-2 `evaluate_what_if` → real engine / out-of-scope decline), ground-never-fabricate (`groundedRefs` audit + detectable violation), plant-scoped bounded entity catalog → change-set construction, persistent+named conversations (migration 0008), graceful degradation (+ `recorded` scripted fallback). 8 API proofs (live Groq) + Copilot UI (grounded data alongside). Constructs + explains; **human applies (D26)**. | **Phase 7** conversational-apply (act on an explored scenario via the conversation, explicit confirm preserving D26); **SSE token-streaming** (turns return JSON + pending UX now); real-time invalidation push (deferred follow-up) | A19 / D55 / D26 · Phase 7 | **Built** (exploration; apply + token-streaming deferred) |
| **SKIP-65** | Conversation streaming transport | Turns return **JSON** with a "Thinking…" pending UX (the streamed feel); the agentic tool-loop is non-streaming and EventSource is GET-only | **SSE token-streaming** of the final prose + tool-phase status events (gateway/provider streams) | A19 · live-demo polish | Deferred (JSON + pending state) |
