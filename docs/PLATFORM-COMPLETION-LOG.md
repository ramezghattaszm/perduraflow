...
| **SKIP-23** | Notification engine | In-app / console only — the TopBar `NotificationBell` + popover UI stub exists (empty/"all caught up" + seeded sample items), presentational only | NotificationDispatcher: rules → recipients → channels → severity; delivery audit; throttle/digest | 14.3 · multi-channel need | Deferred |
...
| **SKIP-53** | Tenant branding management | `tenant.logoUrl` column + `OrgAvatar` render logo-or-placeholder in the shell; `logoUrl`/name set via seed/config (the demo seeds a name, no logo → placeholder shows). Sidebar collapse persisted via a minimal per-user `preferences` JSON on the user row | Admin UI to upload/manage a tenant's logo + wider per-tenant theming (palette/brand overrides); a first-class user-preferences surface | A6 / D34 · admin branding feature | Deferred |
