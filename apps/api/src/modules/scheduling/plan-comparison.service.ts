...
export class PlanComparisonService {
  ...
  async compare(tenantId: string, plantId: string, source: BaselineSource, resourceId?: string): Promise<PlanComparisonDto> {
    ...
    const livePlan = scorePlan(sequence(liveItems, overlay, undefined, cals, ctx.resolveOperatorFactor, ctx.minBatchByResource).placements, { rateByResource, basePlacements: [], overtimeHours: 0, weights: ctx.weights }).kpis
    ...
    if (source === 'frozen_engine_snapshot') {
      ...
      const baseline = scorePlan(sequence(naiveItems, undefined, undefined, cals, ctx.resolveOperatorFactor, ctx.minBatchByResource).placements, { rateByResource, basePlacements: [], overtimeHours: 0, weights: ctx.weights }).kpis
